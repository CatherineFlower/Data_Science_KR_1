    ...
    def _load_columns(self):
        ...
        try:
            ...
            for c in db.list_columns(s, t):
                # Сохраняем весь словарь 'c' (включая data_type)
                self.cbColumn.addItem(c["column_name"], c)
        except Exception as e:
            QMessageBox.critical(self, "Ошибка", f"Не удалось загрузить столбцы: {e}")

    def _do_search(self):
        s, t = self.cbTable.currentData()
        col_data = self.cbColumn.currentData()
        if not col_data:
            QMessageBox.warning(self, "Ошибка", "Выберите столбец.");
            return

        col = col_data["column_name"]
        col_type = col_data.get("data_type", "").lower()

        op = self.cbOp.currentText()
        ...
        if op in OPS and not is_text_type:
            res = QMessageBox.warning(self, "Предупреждение о типе",
                                      f"Вы пытаетесь применить текстовый оператор '{op}'\n"
                                      f"к столбцу '{col}' (тип: {col_type}), который не является текстовым.\n\n"
                                      "PostgreSQL попытается преобразовать тип, но это может быть медленно или привести к ошибке.\n\n"
                                      "Продолжить?",
        ...
        try:
            ...
            if 'invalid regular expression' in err_str:
                QMessageBox.critical(self, "Ошибка",
                                     f"Неверный шаблон: Cинтаксическая ошибка в регулярном выражении.\n\n(Оператор: {op})")
            elif 'operator does not exist' in err_str or 'function does not exist' in err_str:
                QMessageBox.critical(self, "Ошибка",
                                     f"Операция '{op}' несовместима с типом данных столбца '{col}' (тип: {col_type}).")
            elif 'invalid input syntax' in err_str:
                QMessageBox.critical(self, "Ошибка",
